...
// skipframes.cpp
// 
// compile: g++ -m32 -O2 -o skipframes.exe skipframes.cpp -static-libgcc -static-libstdc++ -s
...
bool InjectStandard(DWORD pid, const char* dllPath) {
    ...
    void* pRemotePath = VirtualAllocEx(hProc, NULL, strlen(dllPath) + 1, MEM_COMMIT, PAGE_READWRITE);
    if (!pRemotePath) {
    ...
    HANDLE hThread = CreateRemoteThread(hProc, NULL, 0, (LPTHREAD_START_ROUTINE)pLoadLibrary, pRemotePath, 0, NULL);
    if (!hThread) {
